...
class Distance:
    ...
    def speed(self, dist_left, time):
        ...
        if(dist_left < 0):
            print("done")
            return 0
        elif(dist_left < (self.top_accl/2)*(self.top_speed/self.top_accl)**2):
            print("ramp down")
            return math.sqrt(2*dist_left/self.top_accl)*self.top_accl
...
class MyRobot(wpilib.IterativeRobot):
    ...
    def teleopPeriodic(self):
        ...
        if self.stick.getRawButton(2) and self.toggle:
            ...
            self.wheel2.reset()
            self.start_time = self.tm.getMsClock()
            self.turnController.setSetpoint(self.control.speed(500-average(self.wheel.getDistance(), self.wheel2.getDistance()), (self.tm.getMsClock() - self.start_time)/100))
            self.toggle = False
            rotateToAngle = True
        elif self.stick.getRawButton(2):
            speed = self.control.speed(500-average(self.wheel.getDistance(), self.wheel2.getDistance()), (self.tm.getMsClock() - self.start_time)/100)
            print(speed)
            self.turnController.setSetpoint(speed)
            rotateToAngle = True
