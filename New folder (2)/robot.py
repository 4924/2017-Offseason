...
class Distance:
    ...
    def update(self):
        ...
        if(self.dist_left < (self.top_accl/2)*(self.top_speed/self.top_accl)**2):
            return math.sqrt(self.dist_left/(self.top_accl/2))*self.accl
        elif(self.time > self.top_speed/self.top_accl):
...
class MyRobot(wpilib.SampleRobot):
    ...
    def robotInit(self):
        ...
        self.johnThing = Distance(top_speed, top_accl)
        tm = wpilib.Timer()
        ...
        turnController.setInputRange(-180.0,  500.0)
        ...
        while self.isOperatorControl() and self.isEnabled():
            ...
            if self.stick.getRawButton(2):
                if first:
                    ...
                    self.wheel.reset()
                    self.wheel2.reset()
                self.johnThing.speed((self.wheel.getDistance()+self.wheel2.getDistance())/2, tm.get())
                self.turnController.setSetpoint(self.johnThing.update)
                rotateToAngle = True
